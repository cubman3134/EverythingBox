...
`assets/eb/` under it = the stock `themes2/` plus every first-party addon dir under `native/addons/`.
`QT_ANDROID_PACKAGE_SOURCE_DIR` points at that staged copy, so the committed tree is never mutated and the
generated `assets/eb/` never lands in git. `androiddeployqt` bundles `assets/` into the APK (read-only);
`AssetBootstrap::run("assets:/eb", AppPaths::dataDir(), <version>)` (called from `main.cpp` on first launch)
...
`assets/eb/` is **already wired** (this task): CMake stages it and `AssetBootstrap` extracts it on first run —
