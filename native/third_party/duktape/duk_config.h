...
/* ---- EverythingBox: bounded addon execution time -------------------------------------------------------
 * Addon scripts run synchronously on the GUI thread, so a runaway loop would hang the app. Enable the
 * bytecode-executor timeout check; everythingbox_duk_exec_timeout() (in JsAddon.cpp) returns non-zero once the
 * per-call deadline passes, and Duktape throws a catchable RangeError. The interrupt counter is required.
 */
#if defined(__cplusplus)
extern "C" {
#endif
extern int everythingbox_duk_exec_timeout(void *udata);
#if defined(__cplusplus)
}
#endif
#undef DUK_USE_INTERRUPT_COUNTER
...
#undef DUK_USE_EXEC_TIMEOUT_CHECK
#define DUK_USE_EXEC_TIMEOUT_CHECK(udata)  everythingbox_duk_exec_timeout((udata))
