...
A `helpsystem` chip's `button` is written as the **keyboard** key, and the app re-spells it as a controller button while a pad is driving — following a remap and the connected pad's brand, so a PlayStation user reads `○` where an Xbox user reads `B`. The keys it can translate are `Enter`, `Esc`, `I`, `/`, `F`, `P`, `T`, `S` and `Menu` (`Menu` is the context-menu key, which opens the Emulation settings / queue menu for whatever is focused). Arrow chips pass through as themselves, and so does any other string — a chip only you know about stays exactly as you wrote it rather than being given a button it does not have.
