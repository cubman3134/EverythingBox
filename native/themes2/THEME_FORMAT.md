...
| `actionrow` | the `detail` view's row of action pills — Play/Read, Choose source, Download, Favorite, Playlist, Hide, Status, Tags, the external-player pair. The verbs are chosen per item by the host; ←→ move between them, Enter runs one. **Size it for one row.** A long verb list wraps to as many rows as it needs rather than running off the edge, and whatever you place below the row is moved down to make space, so you do not have to leave a hole for verbs you cannot predict | `fontSize` |
...
A `helpsystem` chip's `button` is written as the **keyboard** key, and the app re-spells it as a controller button while a pad is driving — following a remap and the connected pad's brand, so a PlayStation user reads `○` where an Xbox user reads `B`. The keys it can translate are `Enter`, `Esc`, `I`, `/`, `F`, `P`, `T`, `S` and `M` (`M` opens the Emulation settings / queue menu for whatever is focused; the context-menu key `Menu` does the same and translates too, but most compact keyboards do not have one). Arrow chips pass through as themselves, and so does any other string — a chip only you know about stays exactly as you wrote it rather than being given a button it does not have.
