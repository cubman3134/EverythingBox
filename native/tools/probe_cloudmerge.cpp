...
int main(int argc, char** argv)
{
    ...
    // reachable and every one of them is a DIFFERENT behaviour at the call site, which is why they exist.
    {
        ...
        // assertion in §19-23 that no mutation kills. It stays as the composite tripwire for the day both
    ...
    // ---- 19b. #34 review round 1: UNCONFIRMED edits hold every timer off; a FUTURE stamp is due now ------
    ...
    // are decided in due(), the single choke point every timer and every panel button passes through.
    {
        ...
        // gate is what stopped them and not some other guard answering first — is §19's own schedule
        // assertions, which already pin `one` and `capped` attempting on a manual trigger. Repeating them here
        // would add lines and no kill power. The one case §19 does not carry is a CLEAN record on a manual
    ...
    // ---- 20. #34 resolve(): what an attempt actually does, and why it cannot oscillate -------------------
    {
        ...
        // asserted where it lives, on the baseline write itself, in §23.
    }
    ...
    // ---- 23. #34 review round 1: the pull's FIXED POINT — the half of no-oscillation §20 cannot see ------
    ...
    // adoptSyncedBaseline. Called directly here, with no socket and no Drive account.
    {
        ...
        // round ends. This is the line §20's for-loop was standing in for and could not actually reach.
