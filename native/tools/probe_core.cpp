...
int main(int argc, char** argv) {
    if (argc < 2) { printf("usage: probe <core> [rom]   (set EB_SYSTEM_DIR for cores that need a BIOS)\n"); return 2; }
    LibretroCore core; std::string err;
    if (const char* sd = getenv("EB_SYSTEM_DIR")) { core.systemDir = sd; printf("systemDir: %s\n", sd); }
