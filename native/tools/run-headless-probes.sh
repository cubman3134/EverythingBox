...
if [ "$ra_fail" -eq 0 ]; then
  ...
  # The expression moved into ReaderBridge::settingsRowCount() when issue #136 appended a Select control
  # to the row (the count is that function's business now, and the host feeds the zone from it), so both
  # spellings are accepted. What is gated is unchanged: the BOOK's count still comes from the pure
  # function reading the availability flag, wherever in this file it is written.
  grep -qE 'ReadAloud::bookSettingsRowCount\((bridge_->)?readAloudAvailable\(\)\)' "$RA_HOST" \
    || ra_note "ReaderChromeHost no longer sets the BOOK's readerSettings count from ReadAloud::bookSettingsRowCount(readAloudAvailable()). A literal count and a QML row gated on availability drift apart in exactly one direction: a cursor stop on a control that is not drawn."
  grep -q 'bridge_->settingsRowCount()' "$RA_HOST" \
...
if [ ! -f "$GSM" ]; then
  ...
  GS_TWINS=(
    ...
    'home.rows|new QPushButton(tr("Choose home rows…"))'
    ...
    'remote.filedrop|new QCheckBox(tr("Receive files over the network (file drop)"))'
    ...
    'playon.sendlib|poSend = panelRow(tr("Send Library To Device'
    'wt.open|wtOpen = panelRow(tr("Watch Together'
    'wt.buffering|wtStall = new QComboBox()'
    'livetv.add|tvAdd = panelRow(tr("Add a Live TV Source'
    'chan.bumperchange|cbBrowse = new QPushButton(tr("Change…"))'
    'chan.bumperclear|cbClear = new QPushButton(tr("Clear"))'
    ...
    'downloads.cap|dlCap = new QComboBox()'
    'downloads.removewatched|new QCheckBox(tr("Offer to remove downloads once watched"))'
    ...
    'emu.runahead|runahead = new QComboBox()'
    'emu.rpdriven|rpDriven = new QComboBox()'
    'emu.dosmidi|dosMidi = new QComboBox()'
    ...
    'pb.decorations|bezelPacks = new QPushButton(tr("Browse decoration packs…"))'
    ...
    'reader.zones|readerZones = new QComboBox()'
    'reader.swipe|new QCheckBox(tr("Swipe sideways to turn the page"))'
    'reader.keepawake|new QCheckBox(tr("Keep the screen on while reading"))'
    'reader.dualpage|new QCheckBox(tr("Two pages side by side on a wide screen"))'
    'reader.translate|rlTranslate = new QLineEdit(Settings::readerTranslateEndpoint())'
    'reader.vocab|rlVocab = new QPushButton(tr("Words I looked up…"))'
    ...
    'mal.id|addCredRow(tr("MyAnimeList Client ID:")'
    'mal.secret|addCredRow(tr("MyAnimeList Client secret:")'
    'mal.connect|new QPushButton(MyAnimeListTracker::isConnected() ? tr("Disconnect")'
    'kitsu.email|addCredRow(tr("Kitsu email:")'
    'kitsu.password|addCredRow(tr("Kitsu password:")'
    'kitsu.connect|new QPushButton(KitsuTracker::isConnected() ? tr("Sign out")'
    ...
    'epic.legendary.auth|panelRow(tr("Sign in to Epic (legendary)"))'
    'epic.legendary.get|panelRow(tr("Get legendary"))'
  ...
  GS_CLASSIC_ONLY=(
    ...
    'cbPath = new QLineEdit(Settings::interstitialFolder())'
