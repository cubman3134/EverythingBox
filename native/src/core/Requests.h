...
namespace requests
{
    ...
    // ---- WHEN A STATUS MAY BE ASKED FOR (issue #315) ---------------------------------------------------

    // #109 said the status is fetched "on view", and the themed surface builds a title's detail data for two
    // quite different reasons: because a detail view was OPENED, and because a row was HOVERED (the metadata
    // path re-derives the action row when a stream id bridges in). "On view" read the same on both, so
    // scrolling a shelf of requestable titles issued one GET per row. Bounded and read-only — nothing is
    // submitted, and the once-per-title-per-session guard means a redraw costs nothing — but still traffic
    // to somebody's server generated by cursor movement, and on a long shelf on a slow box it reads as
    // sluggish hovering rather than as a network problem.
    //
    // So the two triggers are separated, and the rule is stated here rather than at the call site because
    // "may I ask the service?" is exactly the kind of decision this file exists to hold.
    enum class StatusTrigger
    {
        Hover,          // a row went under the cursor: draw from what has ALREADY landed, ask nobody
        DetailOpened,   // a detail view was actually opened: this is the "view" #109 meant
    };

    // The one answer. A hover NEVER reaches the network; the status is only SHOWN on a detail view, so that
    // is where it may be fetched. This is deliberately not "is the cache cold" — a cold cache on a hover is
    // still not a reason to ask, and the pill draws its honest "Request" placeholder either way.
