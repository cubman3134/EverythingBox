...
void HomeView::renderRecents()
{
    ...
    // "New" (issue #155) — ONE shelf with TWO producers, on both layouts (this builder fills items_, which
    // is what the themed column renders as well as the classic list).
    //
    ...
    //
    // ABSENT UNLESS THERE IS SOMETHING, the rule every shelf above uses: an install with no follows and no
    ...
    // Past before future, and both after what you were actually watching. "New" leads "Airing Soon" because
    // it is the one of the two you can act on right now — anticipation can wait a row. (It absorbed the
    // "You Missed" shelf when #155 landed; see renderNewShelf for why the two share one strip.)
    renderNewShelf();      // "New" (#155): unseen children of followed series, UNIONED with #25's rows
