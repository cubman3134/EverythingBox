...
| Runahead (`emu/Runahead.h` + `RetroView`): removes a game's own internal input lag by emulating N frames ahead and rolling back; per game (default off, cap 3), refuses rather than slowing the game down | **built**; `probe_runahead` drives the schedule against a fake serializable core — see [**Runahead**](docs/runahead.md) |
| RetroAchievements (`core/Achievements` + `core/Hardcore.h` + `core/Leaderboards.*`): unlocks, opt-in hardcore, **leaderboards** (attempt tracker + per-game list, hardcore-only submission stated rather than failed quietly) and **rich presence** | **built**; `probe_hardcore` pins the affordance policy, `probe_ralead` drives every leaderboard event from a fake source (no account) — see [**RetroAchievements**](docs/retroachievements.md) |
...
native/
  ...
  src/ebook/                EpubBook, Fb2Book, MobiBook/MobiHeader, TextBook, HtmlText, EbookView
                                                      (EPUB/FB2/MOBI-AZW3/text-Markdown-HTML parse + reader)
