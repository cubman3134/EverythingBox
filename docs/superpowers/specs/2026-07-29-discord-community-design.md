...
`tools/discord/` — a new top-level directory. Zero-dependency **Node** using
built-in `fetch` against the Discord REST API: no `npm install`, no lockfile and
no `node_modules` inside a C++/Qt repository.

- **`tools/discord/server.json`** — the declarative structure: categories,
  channels, types, topics, forum tags, roles, permission overwrites, AutoMod
  rules. Committed. This file, not the running server, is the source of truth.
- **`tools/discord/apply.js`** — reads the config and reconciles the server.
  **Idempotent**: match by name, create what is missing, update what has
  drifted, and **never delete**. Re-runnable after any config edit.
...
falling back to `native/secrets/discord.token`. That path is already covered by
the existing `/native/secrets/*` gitignore rule, matching the convention already
used for the ScreenScraper credentials and the Android keystore. The token is
documented in `native/secrets/README.md` alongside those. It is never committed,
logged, or echoed by the script.
...
3. Write `server.json` and `apply.js`; run `--dry-run` and review the plan.
4. Apply. Verify the structure in the client.
5. Configure Onboarding, the GitHub webhooks, and Carl-bot tags.
6. Generate the permanent invite.
7. Land the repo wiring and the in-app link — **last**, because both embed the
   invite URL, which does not exist until step 6.
...
| Bot token leak | Env var or the already-gitignored `native/secrets/` path; never committed or logged |
