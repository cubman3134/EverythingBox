...
- Machine facts (as all tracks): build `cmake --build build --config Release [--target <t>]`; probes need Qt bin (+ /c/mpv-dev) on PATH + offscreen QPA; deploy to C:\EverythingBox-app keeping the exe name; EB_UITEST=1 + uitest.py only; protect user data; `rtk` proxy normal.
